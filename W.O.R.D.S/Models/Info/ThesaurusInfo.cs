...
namespace W.O.R.D.S.Models.Info
{
    public static class ThesaurusInfo
    {
        ...
        public static Dictionary<string, string> Articles = new Dictionary<string, string>
        {
            { "?", "unknown" },
            { "Обвинение", "accuse" },
        ...
        static ThesaurusInfo()
        {
            foreach (KeyValuePair<string, string> keySet in Articles)
            {
                Thesaurus thesaurus = new Thesaurus(keySet.Key, keySet.Value);
            }
        }
    }
}
